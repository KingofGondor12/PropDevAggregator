...
get '/' do
  ...
  @rt_image_search = @rt.xpath('//div[@id="main_content"]//div[@class="topcontent-left"]/article/div[@class="img-container"]/a/img/@src')
  @rt_link_search = @rt.xpath('//div/div//h3/a/@href')
  @rt_snippet_search = @rt.xpath('//div[@id="main_content"]//div[@class="topcontent-left"]/article//div[@class="summary"]')

  @rt_titles = []

  @rt_title_search.each do |div|
    @rt_titles << div.inner_text
  end
  ...
  @rt_link_search.each do |link|
    if link.value.slice(0, 4) != "http"
      link = link.value.insert(0, "http://www.realtytimes.com")
    end
    @rt_links << link
  end
